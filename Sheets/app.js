(function () {
    ...
    var DrawFunctions = (function() {
        ...
        function drawSelectedCellBorder() {
            ...
            borderDiv.style.top = `${pos.y + 129}px`;
            borderDiv.style.width = `${colWidths[selectedCell.col] - 1}px`;
            borderDiv.style.height = `${rowHeights[selectedCell.row] - 1}px`;
            borderDiv.style.border = "3px solid green";
            console.log(`Position for selected cell:`, pos);
        ...
        function drawSelectedCellRange() {
            ...
            selectedRangeDiv.style.top = `${pos.y + 129}px`;
            ...
            console.log(`Range start row: ${rangeStartRow}, col: ${rangeStartCol}`);
console.log(`Position of range start:`, pos);
console.log(`Range Width: ${rangeWidth}, Range Height: ${rangeHeight}`);
    ...
    function run_trim_func(paramString, outputCellName) {
        ...
        // If there isn't exactly one cell, return #NaN
        if (cellNames.length !== 1) return "#NaN";
        
        // Extract the cell name and convert it to row/column
        const c1 = convertCellNameToRowCol(cellNames[0]);
    
        // Track dependencies
        const cName = cellNames[0];
        if (!(cName in executionsDependentOnCell)) {
            executionsDependentOnCell[cName] = [];
        }
        if (!(executionsDependentOnCell[cName].includes(outputCellName))) {
        ...
        // Get the value from the spreadsheet for the single cell
        const v = spreadsheetData[c1.row][c1.col];
        
        // If the value is a string, trim it
        if (typeof v === 'string') {
            const result = v.trim();
            // If trimming results in no change, return the original value
            return result === v ? v : result;
        } else {
            // If the value isn't a string, return #NaN
            return '#NaN';
        }
    }
    ...
    function run_lower_func(paramString, outputCellName) {
        ...
        if (cellNames.length !== 1) return "#NaN";  // LOWER can only be applied to one cell
        ...
        const v = spreadsheetData[c1.row][c1.col];
        if (typeof v === 'string') {
            return v.toLowerCase(); // Convert string to lowercase
        } else {
    ...
    function run_upper_func(paramString, outputCellName) {
        ...
        if (cellNames.length !== 1) return "#NaN";  // UPPER can only be applied to one cell
        ...
        const v = spreadsheetData[c1.row][c1.col];
        if (typeof v === 'string') {
            return v.toUpperCase(); // Convert string to uppercase
        } else {
            return '#NaN'; // Return #NaN if the value is not a string
        }
    }
    ...
    function run_find_and_replace_func(paramString, findText, replaceText, outputCellName) {
        const cellRange = paramString;
        const cellNameRegex = /[A-Z]+\d+/g;
        const cellNames = cellRange.match(cellNameRegex);
    
        if (!cellNames || cellNames.length < 1) return "#NaN"; // Ensure valid cell range
        ...
        for (let cellName of cellNames) {
            ...
            if (!(cellName in executionsDependentOnCell)) {
                executionsDependentOnCell[cellName] = [];
            }
            if (!(executionsDependentOnCell[cellName].includes(outputCellName))) {
                executionsDependentOnCell[cellName].push(outputCellName);
            }
    
            const value = spreadsheetData[c.row][c.col];
            if (typeof value === 'string' && value.includes(findText)) {
                const replacedValue = value.replace(new RegExp(findText, "g"), replaceText);
                modifiedCells[cellName] = replacedValue; // Save modified cell
            } else {
                modifiedCells[cellName] = value; // Keep the original value if no match
            }
        }
    
        // Update the spreadsheet data with modified values
        for (const cellName in modifiedCells) {
            const c = convertCellNameToRowCol(cellName);
            spreadsheetData[c.row][c.col] = modifiedCells[cellName];
        }
    
        return "Done"; // Indicates successful find and replace
    }
    

    const funcList = ["SUM", "AVG", "MIN", "MAX","COUNT","TRIM","UPPER","LOWER"];
    ...
    function displayInput() {
        ...
        textInput.style.top = `${pos.y + 129}px`;
        ...
        textInput.style.outline = "4px solid rgba(4, 60, 32, 0.8)";
